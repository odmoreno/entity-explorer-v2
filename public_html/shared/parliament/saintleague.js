...
const sainteLague = (votes, seats, opt = {}) => {
	...
	const minimumSeatsPerParty = _.fromPairs(Object.entries(votes).map(([party, voteCount]) => {
	...
	const maximumSeatsPerParty = _.fromPairs(Object.entries(votes).map(([party, voteCount]) => {
	...
	const undeterminedQuotients = _.flatMap(Object.entries(votes).map(([party, voteCount]) => {
		const minimumSeats = minimumSeatsPerParty[party]
		const maximumSeats = maximumSeatsPerParty[party]
		return _.range(minimumSeats, maximumSeats).map(seatNumber => {
	...
	const sortedUndeterminedQuotients = _.sortBy(undeterminedQuotients, ({ quotient }) => -quotient)
	...
	if (guaranteedWinners.length + nonGuaranteedWinners.length === seatsLeft) guaranteedWinners.push(...nonGuaranteedWinners)
		...
		guaranteedWinners.push(..._.sampleSize(nonGuaranteedWinners, seatsLeft - guaranteedWinners.length))
	}
